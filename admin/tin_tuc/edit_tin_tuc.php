            ...
            if ($img["size"] > $maxSize) {
                ...
                if ($check === false) {
                    ...
                    if ($err_tieu_de . $err_noi_dung . $error_hinh_anh . $err_tom_tat == "") {
                        insert_delete_update("UPDATE tin_tuc SET tieu_de = '$tieu_de',noi_dung = '$noi_dung', thoi_gian = '$thoi_gian', hinh_anh = '$imgname', tom_tat = '$tom_tat', id_user = $id_user WHERE id = $id");
                        header("Location:index.php?msg=Cập nhập tin tức thành công !");
                    }
                }
            }
        } else {
            if ($err_tieu_de . $err_noi_dung . $error_hinh_anh . $err_tom_tat == "") {
insert_delete_update("UPDATE tin_tuc SET tieu_de = '$tieu_de',noi_dung = '$noi_dung', thoi_gian = '$thoi_gian', tom_tat = '$tom_tat', id_user = $id_user WHERE id = $id");
                header("Location:index.php?msg=Cập nhập tin tức thành công !");
            }
        }
    }
                        ...
                        <form action="" method="post" enctype="multipart/form-data" class="form_user">
                            <div class="row">
                                <div class="col-md-12" style="padding:0 25px 0 25px;">
                                    <?php
                                    foreach (selectAll("SELECT * FROM tin_tuc WHERE id = '$id'") as $item) {
                                    ?>
                                        <div class="form-group">
                                            <label for="">Tiêu đề *</label>
                                            <input type="text" value="<?= $item['tieu_de'] ?>" class="form-control" name="tieu_de" placeholder="Tiêu đề" style="background-color: #fdf7f7;">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" id="full-featured-non-premium" name="noi_dung" placeholder="Nội dung" id="full-featured-non-premium"><?= $item['noi_dung'] ?></textarea>
                                        ...
                                        <img src="<?= BASE_URL ?>content/images_tin_tuc/<?= $item['hinh_anh'] ?>" width="50%" alt="">
                                        <br><br>
                                        <div class="form-group">
                                            <label for="">Tóm tắt *</label>
                                            <textarea class="form-control" name="tom_tat" placeholder="Tóm tắt" style="background-color: #fdf7f7;"><?= $item['tom_tat'] ?></textarea>
<?php if (isset($err_tom_tat)) { ?>
                                                <span style="color:red;">
                                                    <?php echo $err_tom_tat . "<br>" ?></span>
                                            <?php } ?>
                                        </div>
                                        <button type="submit" class="btn btn-secondary" name="add_tin_tuc">Cập nhập</button>
